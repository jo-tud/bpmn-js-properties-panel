...
function domQueryAll(selector, context) {
  return [].slice.apply(domQuery.all(selector, context));
}
...
PropertiesPanel.prototype._updateActivation = function(current) {
  ...
  forEach(current.tabs, function(tab) {
    ...
    var tabLinkNode = domQuery('.djs-properties-tabs-links [data-tab-target=' + tab.id + ']').parentNode;
...
PropertiesPanel.prototype._createPanel = function(element, tabs) {
  ...
  forEach(tabs, function(tab, _t) {
    ...
    domQuery('a', tabLinkNode).addEventListener('click', function (evt) {
      ...
      domQueryAll('a', tabLinksNode).forEach(function (el) {
        domClasses(el.parentNode).remove('active');
      });

      domQueryAll('.djs-properties-tab[data-tab]', tabContainerNode).forEach(function (el) {
