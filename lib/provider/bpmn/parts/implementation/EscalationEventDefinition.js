...
function updateOptionsDropDown(businessObject, referencedType, entryNode) {
  ...
  var selectBox = domQuery('select[name=escalations]', entryNode);
...
function EscalationEventDefinition(group, element, bpmnFactory, escalationEventDefinition) {
  group.entries.push({
    ...
    addEscalation: function(element, inputNode) {
      ...
      var newEscalation = elementHelper.createElement('bpmn:Escalation', {}, utils.getRoot(escalationEventDefinition), bpmnFactory);
    ...
    get: function(element, entryNode) {
      ...
      var options = updateOptionsDropDown(escalationEventDefinition, 'bpmn:Escalation', entryNode);
